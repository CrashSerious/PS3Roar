...
static void find_working_machine(void) {
	char *tmp = NULL;
	LOG_DEBUG(3, "****Finding working Machine.\n");
	LOG_DEBUG(3, "requested_config.extbas_rom= %s\n", requested_config.extbas_rom);
	LOG_DEBUG(3, "requested_config.bas_rom= %s\n",requested_config.bas_rom);
	...
	if ((tmp = find_rom_in_list(requested_config.extbas_rom, d64_extbas_roms))) {
		...
		if (requested_config.tv_standard == TV_NTSC)
			requested_machine = MACHINE_COCOUS;
				...
				//LOG_DEBUG(3, "Make it Coco.\n");
				requested_machine = MACHINE_COCO;
		}
	} else {
		/* Fall back to this, which won't start up properly */
			LOG_DEBUG(3, "Falling Back. Parachute. Epic Fail.\n");
			requested_machine = MACHINE_COCO;//MACHINE_DRAGON64;
	}
	if (tmp)
		free(tmp);
}
...
static char *find_rom_in_list(const char *preferred, const char **list) {
	char *path;
	int i;
	//LOG_DEBUG(3, "in find_rom_in_list...[%s %s]\n", preferred, path);
	if (preferred && (path = find_rom(preferred))){
		//LOG_DEBUG(3, "returning path from find_rom_in_list: %s %sROM.\n", preferred, path);
		return path;
	}
	if (list == NULL){
		//LOG_DEBUG(3, "List is null, returning same.\n", preferred);
		return NULL;
	}
	for (i = 0; list[i]; i++) {
		//LOG_DEBUG(3, "in the for loop...\n", preferred);
		//LOG_DEBUG(3, "loop processing list[i] = %s...\n", list[i]);
		if ((path = find_rom(list[i])))
			return path;
	}
	//LOG_DEBUG(3, "find_rom_in_list: completed. Exiting...\n", preferred);
	return NULL;
}
...
static int load_rom_from_list(const char *preferred, const char **list,
	...
	char *path;
	int ret;
	//LOG_DEBUG(3, "Loading %s ROM.\n", preferred);

	path = find_rom_in_list(preferred, list);
	//LOG_DEBUG(3, "Loading ROM from %s.\n", path);
...
int machine_load_rom(const char *path, uint8_t *dest, size_t max_size) {
	...
	//LOG_DEBUG(3, "Checking %s for ROM.\n", path);
